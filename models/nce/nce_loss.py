...
class NCELoss(nn.Module):
    ...
    def forward(self, target, *args, **kwargs):
        ...
        if self.loss_type != 'full':
            ...
            # B,N,Nr
            logit_noise_in_noise = self.logprob_noise[noise_samples.data.contiguous().view(-1)].contiguous().view_as(noise_samples)
            logit_target_in_noise = self.logprob_noise[target.data.contiguous().view(-1)].contiguous().view_as(target)
